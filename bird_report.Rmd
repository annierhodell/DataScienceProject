...
```{r niche-table_data, echo=TRUE}
niche_table <- tibble(
  trophic_niche = c("Frugivore", "Granivore", "Nectarivore", "Herbivore", "Herbivore Aquatic", "Invertivore", "Vertivore", "Aquatic Predator", "Scavenger", "Omnivore"),
  diet_description = c("fruit", "seeds or nuts", "nectar", "plant materials in non-aquatic systems, including leaves, buds, whole flowers etc.", "plant materials in aquatic systems, including algae and aquatic plant leaves.", "invertebrates in terrestrial systems, including insects, worms, arachnids, etc.", "vertebrate animals in terrestrial systems, including mammals, birds, reptiles etc.", "vertebrate and invertebrate animals in aquatic systems, including fish, crustacea, molluscs, etc.", "carrion, offal or refuse.", "Species using multiple niches, within or across trophic levels, in relatively equal proportions.")
  )
```

```{r niche-table, echo = FALSE}
kable(head(niche_table), format = "html") %>%
  print(n = 10)
```
...
heres a graph just showing the means
...
birds_data %>%
  ...
  geom_col(mapping = aes(y = Niche_Trophic,
                         ...
                         fill = variable),
           position = "dodge")+
  labs( title = "The Mean Beak Measurements of each Niche Trophic Level",
        x = "Mean Measurement in mm",
        y = "Niche Trophic Level")+
  scale_fill_manual(name = "Type of Beak Measurement",
                    values = c("Width" = "blue", "Length" = "green", "Depth" = "red"))
        #the colours need changing


```

see about the mean etc

pie chart about niche levels

```{r include=FALSE, piechartniche, echo=FALSE}
#Pie chart of the Niche trophic levels proportions
...
```
As u can see there is alot about Invertivores, so choose that one.
...
Our first model, model-1, uses only beak depth as a predictor value to predict whether the bird is an Invertivore or not. We then made a second model for Invertivores that uses all 3 predictor values (beak length, width and depth).
...
#needs to be seen in report somehow

```

```{r model-o, include=FALSE}

#model for nectarivores maybe?
...
The thought behind out model is that birds beaks have evolved and taken on different beak shapes to best eat there chosen food type. Models like this could be used in conjunction with other methods.

Limitations:

model is binomial, can only predict success or failure for one food type. Cant predict from a wide variety. 
Model also isnt very good, would be better if if is within a certain range, or proportion. e.g beak width in proportion to beak length, might get more accurate results.
look at cutoff prob table. There is alot of false positives.
