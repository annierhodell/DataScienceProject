...
- They eat invertibrates such as insects, worms and arachnids
- More than 60% of its diet is made up of these invertibrates
...
##We modeled Invertivore based on just the beak length to see if it had a similar effect
...
- This demonstrates that modeling Invertivore status using all three predictors for beak data is a better model and has a higher predictive power than just using the peak length.
---
background-image: url(https://images.unsplash.com/photo-1470167494176-c2e966a4eb3d?q=80&w=2342&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D)
...
-Another limitation of our model is that, since a logistic model can only predict yes or no, and can't predict multiple variables simultaneously so we can't predict which type of niche trophic the bird is if it isn't a invertivore, only that it isn't 
