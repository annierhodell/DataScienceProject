...
- HBT_area_names_and_population: We made this data set to contain the area names and populations corresponding to the given HBT codes in the original data set so we could join them together. I.e. S08000015 is the HBT code for the area Ayrshire and Arran. 

- Treatment_Location_names: We made this data set to contain the treatment location names corresponding to the given treatment location codes in the original data set so we could join them together. I.e. A111H is the treatment location code for University Hospital Crosshouse.
...
The begin our investigation, we first had to tidy our main data set. This process consisted of using data science techniques in order to make our data easier to be analysed. 

We selected the specific columns we needed to reduce the overall data size making it easier to manage. In order to add in the missing percentage over 4 hours column, we used the mutate() function. Mutating was also required to format the data type of the week ending date column from double to date. We joined our data set with both the HBT_area_names_and_population and Treatment_Location_names data sets using the left_join() function. 

After reordering and renaming the columns, our tidied data set is called data.

We used a similar process to tidy our second data set, a_and_e_hb_agesex_20231005. This data set is now called Age_and_Sex_Data.

### Techniques

Throughout our investigation we used a variety of data science techniques, these included:
